...
<div>
  ...
  <div class="container flex flex-col">
    ...
    <div class="m-5 flex">
      ...
      <select v-model="bodyPart" id="small" class="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-900 bg-gray-100 border border-gray-300 rounded-l-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 focus:border-primary" placeholder="All Exercises">
      ...
      <div class="relative w-full">
        <input v-model="searchQuery" type="search" class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-r-lg border-l-gray-50 border-l-2 border border-gray-300 focus:ring-primary focus:border-primary " placeholder="Search by Exercise name ..." >
        <button type="submit" class="absolute top-0 right-0 p-2.5 text-sm font-medium h-full text-white bg-primary rounded-r-lg border border-primary hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 ">
    ...
    <button @click="loadMore" class="btn-more-to-explore w-40 h-12 self-center m-5">
      More To Explore
    </button>
...
let exercises = computed(() => {
   ...
   .filter(exercise => exercise.name.toLowerCase().includes(searchQuery.value.toLowerCase()))
   .filter(exercise => bodyPart.value === '' || exercise.bodyPart.toLowerCase() === bodyPart.value.toLowerCase()) 
